...
Let's begin by installing [MySQL Server](https://dev.mysql.com/downloads/mysql/) and [MySQL Workbench](https://dev.mysql.com/downloads/workbench/). Make sure the server is running and then start Workbench and connect to the server. In the query interface, we will experiment with the basic SQL commands.
